...
export async function getFullResidentHistory() {
  ...
  const egyptians = await db.select({
    id: egyptianResidents.id,
    name: egyptianResidents.name,
    idNumber: egyptianResidents.nationalId,
    phone: egyptianResidents.phone,
    unitId: egyptianResidents.unitId,
    checkInDate: egyptianResidents.checkInDate,
    checkOutDate: egyptianResidents.checkOutDate,
    status: egyptianResidents.status,
    type: sql`'egyptian'`
  }).from(egyptianResidents);

  const russians = await db.select({
    id: russianResidents.id,
    name: russianResidents.name,
    idNumber: russianResidents.passportNumber,
    phone: russianResidents.phone,
    unitId: russianResidents.unitId,
    checkInDate: russianResidents.checkInDate,
    checkOutDate: russianResidents.checkOutDate,
    status: russianResidents.status,
    type: sql`'russian'`
  }).from(russianResidents);

  const allResidents = [...egyptians, ...russians];
  const unitIds = [...new Set(allResidents.map(r => r.unitId).filter(Boolean) as number[])];
  
  let unitsData: any[] = [];
  if (unitIds.length > 0) {
    unitsData = await db.select().from(units).where(inArray(units.id, unitIds));
  }

  return allResidents.map(r => {
    const unit = unitsData.find(u => u.id === r.unitId);
    return {
      ...r,
      unitCode: unit?.code || "غير معروف",
      buildingName: unit?.buildingName || "-",
    };
  }).sort((a, b) => Number(b.checkInDate || 0) - Number(a.checkInDate || 0));
}
...
export async function getOccupancyStats() {
  ...
  return allUnits.map(u => {
    const occupantsCount = 
      activeEgyptians.filter(r => r.unitId === u.id).length + 
      activeRussians.filter(r => r.unitId === u.id).length;
    ...
    return {
      ...
      totalBeds: u.beds,
      occupiedBeds: occupantsCount,
      vacantBeds: Math.max(0, u.beds - occupantsCount),
      status: u.status
    };
  });
}

export async function getDetailedUnitReport() {
  const db = await getDb();
  if (!db) return [];
  ...
  const activeEgyptians = await db.select().from(egyptianResidents).where(eq(egyptianResidents.status, "active"));
  const activeRussians = await db.select().from(russianResidents).where(eq(russianResidents.status, "active"));
  const historyEgyptians = await db.select().from(egyptianResidents).where(eq(egyptianResidents.status, "checked_out"));
  const historyRussians = await db.select().from(russianResidents).where(eq(russianResidents.status, "checked_out"));

  return allUnits.map(u => {
    const current = [
      ...activeEgyptians.filter(r => r.unitId === u.id).map(r => ({ name: r.name, id: r.nationalId, phone: r.phone, date: r.checkInDate })),
      ...activeRussians.filter(r => r.unitId === u.id).map(r => ({ name: r.name, id: r.passportNumber, phone: r.phone, date: r.checkInDate }))
    ];

    const history = [
      ...historyEgyptians.filter(r => r.unitId === u.id).map(r => ({ name: r.name, id: r.nationalId, phone: r.phone, date: r.checkOutDate })),
      ...historyRussians.filter(r => r.unitId === u.id).map(r => ({ name: r.name, id: r.passportNumber, phone: r.phone, date: r.checkOutDate }))
    ];
